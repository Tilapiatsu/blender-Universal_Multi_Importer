...
def zipdir(path: Path, ziph: zipfile.ZipFile, zip_subdir_name, ignore_folders: tuple):
...
def generate_zip_filename(addon_name: str, zip_suffix: str) -> str:
    ...
    return "{}{}_v{}.{}.{}.zip".format(addon_name, zip_suffix, major, minor, patch)
...
def zip_main(addon_name: str, ignore_folders: tuple, zip_suffix: str = ""):
    """Zip the addon."""
    filename = generate_zip_filename(addon_name, zip_suffix)
    ...
    try:
        ...
        zipdir(get_addon_path(addon_name), zipf, lower_name, ignore_folders=ignore_folders)
        zipf.close()
