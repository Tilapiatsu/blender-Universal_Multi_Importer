# CREDIT: https://projects.blender.org/blender/blender/src/branch/main/tests/utils/bl_run_operators_event_simulate.py
...
class action_handlers:
    ...
    # ADDED
    @staticmethod
    def screenshot(path, *, only_validate=False, **kw):
        """Takes a screenshot, saves to filepath."""
        yield from action_handlers.event(type='F24', value='PRESS', only_validate=only_validate)
        ...
        bpy.ops.screen.screenshot(
            'EXEC_DEFAULT',
            filepath=str(os.path.join(os.path.dirname(__file__), path)),
        )
    # END ADDED
...
if __name__ == "__main__":
    # ADDED, install the add-on
    from importlib.machinery import SourceFileLoader
    zip_addon = SourceFileLoader(
        "zip_addon",
        os.path.join(os.path.dirname(__file__), "zip_addon.py")
    ).load_module()
    
    addon_zip_file = zip_addon.zip_main('addon_testing')
    bpy.ops.preferences.addon_install(filepath=addon_zip_file, enable_on_install=True)
    # END ADDED
    main()

    # ADDED
    os.unlink(addon_zip_file)
    # END ADDED
