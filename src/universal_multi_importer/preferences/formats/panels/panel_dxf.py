...
class IMPORT_SCENE_DXFSettings:
    ...
    def draw(self, operator, module_name, layout):
        ...
        if BVERSION >= 4.2:
            op = [[operator, "scene_options"], [operator, "collection_options"]]

            draw_panel(layout, op, "DXFSettings_Import", "Import Options", icon="IMPORT")

            op = [[operator, "block_options"], [operator, "do_bbox"], [operator, "merge"]]
            ...
            op = [[operator, "import_text"], [operator, "import_light"], [operator, "export_acis"]]

            draw_panel(layout, op, "DXFSettings_OptionalObjects", "Optional Objects", icon="IMPORT")

            op = [[operator, "outliner_groups"], [operator, "recenter"]]

            draw_panel(layout, op, "DXFSettings_ViewOptions", "View Options", icon="RESTRICT_VIEW_OFF")

            op = [[operator, "dxf_indi"], [operator, "dxf_scale"]]

            _, panel = draw_panel(
                layout,
                op,
                "DXFSettings_GeoReferencing",
                "Geo Referencing",
                set_header_boolean=True,
                header_bool=[operator, "use_georeferencing"],
            )
            ...
            op = [[operator, "outliner_groups"], [operator, "create_new_scene"], [operator, "recenter"]]
