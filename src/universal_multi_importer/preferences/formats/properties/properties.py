...
class PG_GlobalSettings(bpy.types.PropertyGroup):
    import_simultaneously_count: bpy.props.IntProperty(
        name="Max Simultaneously Files",
        default=200,
        min=1,
        description="Maximum number of file to import simultaneously",
    )
    max_batch_size: bpy.props.FloatProperty(
        name="Max batch size (MB)",
        description="Max size per import batch. An import batch represents the number of files imported simultaneously",
        default=20,
        min=0,
    )
    minimize_batch_number: bpy.props.BoolProperty(
        name="Minimize batch number",
        description="Try to pack files per batch in a way to be as close as possible to the Max batch size, and then minimize the number of import batches",
        default=True,
    )
    create_collection_per_file: bpy.props.BoolProperty(
        name="Create collection per file",
        description="Each imported file will be placed in a collection",
        default=False,
    )
    recreate_folder_structure_as_collections: bpy.props.BoolProperty(
        name="Recreate folder structure as collections",
        description="Recreate the folder structure from the hard drive into blender collections",
        default=False,
    )
    backup_file_after_import: bpy.props.BoolProperty(
        name="Backup file during import",
        description='Backup file after importing file. The frequency will be made based on "Bakup Step Parameter"',
        default=False,
    )
    backup_step: bpy.props.FloatProperty(
        name="Backup Step (MB)", description="Backup file after X file(s) imported", default=100, min=1
    )
    skip_already_imported_files: bpy.props.BoolProperty(
        name="Skip already imported files",
        description='Import will be skipped if a Collection with the same name is found in the Blend file. "Create collection per file" need to be enabled',
        default=False,
    )
    save_file_after_import: bpy.props.BoolProperty(
        name="Save file after import completed",
        description="Save the original file when the entire import process is complete",
        default=False,
    )
    ignore_command_batcher_errors: bpy.props.BoolProperty(
        name="Ignore Command Batcher Errors",
        default=True,
        description="Disable it if you want to stop the process if an error occurs in the Command Batcher",
    )
    ...
    force_refresh_viewport_after_each_import: bpy.props.BoolProperty(
        name="Refresh Viewport After Each Imported Files", default=False
    )
    force_refresh_viewport_after_each_command: bpy.props.BoolProperty(
        name="Refresh Viewport After Each Processed Command", default=True
    )
    force_refresh_viewport_after_time: bpy.props.FloatProperty(
        name="Refresh Viewport After time (s)",
        default=1.0,
        min=0,
        description="The viewport will refresh after the X seconds. It help to control viewport interactivity. A value of 0 will disable it, and the viewport will refresh after each batch",
    )
    display_debug_log: bpy.props.BoolProperty(name="Display Debug Log", default=False)
...
class PG_UMISettings(bpy.types.PropertyGroup):
    ...
    umi_current_format_setting_cancelled: bpy.props.BoolProperty(
        name="Current Format Settings cancelled", default=False
    )
    ...
    umi_command_batcher_settings: bpy.props.EnumProperty(
        items=[
            ...
            ("EACH_ELEMENTS", "Each Elements", ""),
            ("POST_PROCESS", "Post-Process", ""),
        ],
        default="EACH_ELEMENTS",
    )
    ...
    umi_md5_generation_status: bpy.props.EnumProperty(
        name="MD5 Generation Status",
        default="NOT_STARTED",
