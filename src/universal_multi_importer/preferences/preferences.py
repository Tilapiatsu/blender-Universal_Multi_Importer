...
def draw_addon_formats(context, layout, addon_dependencies, umi_settings):
    ...
    if not len(addon_dependencies):
        ...
        umi_settings.umi_all_addon_dependencies_installed
        and umi_settings.umi_all_addon_dependencies_enabled
        and umi_settings.umi_addon_dependency_need_reboot
    ):
        bbox.label(
            text=f"All formats are installed/enabled/updated properly, but before it fully works, you will have to :",
            icon="ERROR",
        )
        ...
        col.alert = True
        col.label(
            text=f"Install/Enable/Update the formats by clicking on the install/enable/update button below",
            icon="RADIOBUT_ON",
        )
        col.label(text=f"Disable Universal Multi Importer Addon", icon="RADIOBUT_ON")
        col.label(text=f"Restart Blender", icon="RADIOBUT_ON")
    ...
    for ad in addon_dependencies.values():
        ...
        if not ad.is_installed and len(ad.addon_name) > 0:
            ...
            if not context.preferences.system.use_online_access:
                grid_layout(row, alignment="CENTER", size=6).operator(
                    "extensions.userpref_allow_online", text=version, icon="INTERNET_OFFLINE"
                )
            else:
                op = grid_layout(row, alignment="CENTER", size=6).operator(
                    "extensions.umi_install_extension", text=version, icon="FILE_REFRESH"
                )
                op.pkg_id = ad.pkg_id
                op.repo_index = 0
        ...
        # installed
            ...
            if not context.preferences.system.use_online_access:
                ...
                op = grid_layout(row, alignment="CENTER", size=4).operator(
                    "extensions.umi_install_extension", text=f"Install {name}"
                )
                op.pkg_id = ad.pkg_id
                op.repo_index = 0
