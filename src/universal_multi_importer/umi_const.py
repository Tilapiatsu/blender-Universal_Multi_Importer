...
def get_datalist():
    ...
    if BVERSION >= 4.1:
        ...
        datatype_list.append(
            {"name": "lightprobes", "icon": "LIGHTPROBE_CUBEMAP"},
        )
...
def get_datatype_properties():
    ...
    for d in DATATYPE_LIST:
        ...
        datatype_result += (
            {
                "property": f"{DATATYPE_PREFIX}_{d['name']}",
                "type": "BOOLEAN",
                "default": default,
            ...
            datatype_result += (
                {
                    ...
                    "type": "ENUM",
                    "items": items,
                    "default": MODIFIER_TYPES[0],
                    "name": d["name"].replace("_", " ").title() + " Type",
                    "description": "Restrict the execution of the command to a specific modifier type",
                    "set": None,
                    "get": None,
                    "icon": d["icon"],
                },
...
def get_operator_items(self, context):
    return [
        ...
        if not d.startswith("_") and isinstance(getattr(bpy.data, d), bpy.types.bpy_prop_collection)
    ]
