...
def draw_custom_panel(layout, idname, header_name: str, icon="NONE") -> tuple:
    ...
    def __wrapper(self, operator, module_name, layout):
        ...
        if not addon_version.is_installed:
            ...
            if panel:
                panel.alert = True
                panel.label(
                    text=f"{operator.name} addon is not installed, you can fix dependencies with the button bellow"
                )
            ...
            if panel:
                ...
                if addon_version.local_version > supported_version:
                    ...
                    op = panel.operator(
                        "extensions.umi_install_extension", icon="FILE_REFRESH", text="Update Universal Multi Importer"
                    )
                    ...
                    op = panel.operator(
                        "extensions.umi_install_extension",
                        icon="FILE_REFRESH",
                        text=f"Update {addon_version.pkg_name} addon",
                    )
            ...
            if panel:
                panel.alert = True
                panel.label(
                    text=f"{addon_version.pkg_name} is disable, please enable it before moving forward",
                    icon=WARNING_ICON,
                )
                panel.operator(
                    "preferences.umi_addon_enable", icon="CHECKMARK", text=f"Enable {addon_version.pkg_name} addon"
                )

        try:
            return f(self, operator, module_name, layout)
        except ValueError as e:
            header, panel = draw_custom_panel(
                layout, "ImportSettingMismatch", "Import Settings not found", icon=WARNING_ICON
            )
            header.alert = True
            if panel:
                panel.alert = True
                ...
                box.operator(
                    "wm.url_open", icon=WARNING_ICON, text="Report Issue"
                ).url = "https://github.com/Tilapiatsu/blender-Universal_Multi_Importer/issues/new"
