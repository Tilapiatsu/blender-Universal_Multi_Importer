...
class IMPORT_SCENE_OT_tila_import_blend(bpy.types.Operator):
    ...
    import_module: bpy.props.EnumProperty(
        name="Import Module", default="APPEND", items=[("APPEND", "Append", ""), ("LINK", "Link", "")]
    )
    ...
    import_brushes: bpy.props.BoolProperty(name="Brushes", default=False)
    import_cameras: bpy.props.BoolProperty(name="Cameras", default=False)
    import_cache_files: bpy.props.BoolProperty(name="Cache files", default=False)
    import_curves: bpy.props.BoolProperty(name="Curves", default=False)
    import_hair_curves: bpy.props.BoolProperty(name="Hair Curves", default=False)
    import_fonts: bpy.props.BoolProperty(name="Fonts", default=False)
    import_grease_pencils: bpy.props.BoolProperty(name="Grease Penil", default=False)
    import_collections: bpy.props.BoolProperty(name="Collections", default=False)
    import_images: bpy.props.BoolProperty(name="Images", default=False)
    import_lights: bpy.props.BoolProperty(name="Lights", default=False)
    import_linestyles: bpy.props.BoolProperty(name="Line Styes", default=False)
    import_lattices: bpy.props.BoolProperty(name="Lattices", default=False)
    import_masks: bpy.props.BoolProperty(name="Masks", default=False)
    import_materials: bpy.props.BoolProperty(name="Materials", default=False)
    import_metaballs: bpy.props.BoolProperty(name="Metaballs", default=False)
    import_meshes: bpy.props.BoolProperty(name="Meshes", default=False)
    import_movieclips: bpy.props.BoolProperty(name="Movie Clips", default=False)
    import_node_groups: bpy.props.BoolProperty(name="Node Groups", default=False)
    import_objects: bpy.props.BoolProperty(name="Objects", default=True)
    import_paint_curves: bpy.props.BoolProperty(name="Paint Cures", default=False)
    import_palettes: bpy.props.BoolProperty(name="Palettes", default=False)
    import_particles: bpy.props.BoolProperty(name="Particles", default=False)
    ...
    import_textures: bpy.props.BoolProperty(name="Textures", default=False)
    ...
    def import_datas(self):
        if self._import_datas is None:
            self._import_datas = [
                m.replace("import_", "")
                for m in self.__annotations__.keys()
                if m.startswith("import_") and m != "import_module" and getattr(self, m)
            ]
    ...
    def local_names(self):
        if self._local_names is None:
            ...
            for datatype in dir(bpy.data):
                if (
                    datatype in self.excluded_datatypes or datatype.startswith("__")
                ) or datatype not in self.import_datas:
                    continue
            ...
            for o in library_objects:
                ...
                if o.name in self.current_collection.objects:
                    ...
                    if library_duplicate:
                        LOG.warning(
                            f'Blend format : "{o.name}" {source_string} already in "{self.current_collection.name}" collection. Skipping ...'
                        )
                        continue

                LOG.info(
                    f'Blend format : Link "{o.name}" {source_string} to "{self.current_collection.name}" collection'
                )
