...
def get_file_selection():
    ...
    size = len(file_selection)

    active = file_selection[idx] if (size and size > idx - 1) else None
...
class UI_Select(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if self.mode == "ALL":
            ...
            for f in self.file_selection:
                if (
                    f.size > self.umi_settings.umi_file_size_min_selection
                    and f.size < self.umi_settings.umi_file_size_max_selection
                ):
                    f.check = self.bool_action
        elif self.mode == "NAME":
            for f in self.file_selection:
