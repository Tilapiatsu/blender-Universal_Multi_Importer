...
COMMAND_BATCHER_INPUT_ITEMS = [
    (
        "ITEM_NAME",
        "Item Name (str)",
        "Returns the name of the item. The item depends on what is being processed : If the processed item is an object, it will returns the object name. If the processed item is a material, it will return the material name etc ...",
    ),
    (
        "GLOBAL_ITEM_INDEX",
        "Global Item Index (int)",
        "Returns a integer from 0 to n where 0 is the first item processed and n the last item.",
    ),
    (
        "DATA_ITEM_INDEX",
        "Item Index (int)",
        "Returns a integer from 0 to n where 0 is the first item processed and n the last item of a given Datatype.",
    ),
    (
        "ITEM_DATA",
        "Item Data (str)",
        r"Returns a string that point to the data of the currently processed item. If the processed item is an object, it will return bpy.data.objects[ObjectName],  and If the processed item is a material, it will return bpy.data.materials[MaterialName].\nIt allows you to access the data of the currently processed item and modify it the way you want",
    ),
    (
        "OBJECTS",
        "Objects (list)",
        "Returns a list of objects using the given data. The Command will be executed for each objects in the list",
    ),
    (
        "OBJECT_BBOX",
        "Object Bounding Box Size (vector3)",
        "Works if the processed item is an object : It returns a Vector3 that represent the bounding box size of the object.",
    ),
...
def get_command_batcher_variable():
    variables = {
        v[1]: {
            "default": f'"<{v[0]}>"',
            "type": "STRING",
            "property": f"{COMMAND_BATCHER_VARIABLE_PREFIX}_{v[0].lower()}",
            "name": v[1],
            "description": v[2],
            "option": {"SKIP_SAVE"},
            "set": revert_to_default,
            "get": get_value(f"<{v[0]}>"),
        }
        for v in COMMAND_BATCHER_INPUT_ITEMS
    }
    return variables
...
def get_bound_box_size(obj) -> Vector:
    ...
    for v in bound_box:
        bbox_min = Vector(
            (
                min(bbox_min[0], v[0] * obj.scale[0]),
                min(bbox_min[1], v[1] * obj.scale[1]),
                min(bbox_min[2], v[2] * obj.scale[2]),
            )
        )
        bbox_max = Vector(
            (
                max(bbox_max[0], v[0] * obj.scale[0]),
                max(bbox_max[1], v[1] * obj.scale[1]),
                max(bbox_max[2], v[2] * obj.scale[2]),
            )
        )
