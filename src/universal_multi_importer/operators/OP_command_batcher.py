...
def draw_operators(self, context, layout, operators, operator_name: str, operator_id_name: str) -> None:
    ...
    col1.template_list(
        "UMI_UL_operator_list", "", self.umi_settings, operator_name, self.umi_settings, operator_id_name, rows=rows
    )
...
def draw_command_batcher(self, context, layout):
    ...
    row = box.row()
    row.template_list(
        "UMI_UL_preset_list", "", self.umi_settings, "umi_presets", self.umi_settings, "umi_preset_idx", rows=rows
    )
    col2 = row.column()
    col2.separator()
...
class CommandBatcher(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if event.type == "TIMER":
            ...
            if (
                not self.pre_processing
                and not self.processing
                and self.current_element_to_process is None
                and self.element_to_process_count
            ):  # Process can start
                self.next_object()
                ...
                self.current_element_to_process is None
                and self.element_to_process_count == 0
                ...
                self.current_element_to_process is None
                and self.element_to_process_count == 0
                and self.post_process_done
            ):
                if not self.importer_mode:
                    LOG.complete_progress_importer(
                        show_successes=False, duration=round(time.perf_counter() - self.start_time, 2)
                    )
                    self.counter = self.umi_settings.umi_global_import_settings.wait_before_hiding
                else:
                    self.finished = True
            ...
            # Each-Process Done
                ...
                not self.each_process_done
                and self.pre_process_done
                and self.current_command is None
                and not len(self.operators_to_process)
            ):
    ...
    def execute(self, context):
        ...
        self.number_of_operations_to_perform = (
            number_of_operations * number_of_elements
            + len(self.pre_operators_to_process)
